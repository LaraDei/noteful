...
import MainNotePage from './MainNotePage'

describe(`MainNotePage component`, () => {
  const props = {
    note: {
  ...
  it('renders a .NotePageMain by default', () => {
    const wrapper = shallow(<MainNotePage />)
    expect(toJson(wrapper)).toMatchSnapshot()
  })

  it('renders a Note with note prop', () => {
    const note = shallow(<MainNotePage {...props} />)
      .find('Note')
  ...
  it(`splits the content by \\n or \\n\\r, with a p foreach`, () => {
    [{
      ...
      const content = shallow(<MainNotePage {...props} />)
